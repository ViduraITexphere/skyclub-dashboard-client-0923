...
function AddPlace() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('https://skyclub-dashboard-server-0923.vercel.app/api/places', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          navigate('/places'); // Redirect after success message
      ...
      toast.error('Error adding place:', error); // Error message
    }
  };
